<?php
$protectedProcs = array('sp_BlankDelete', 'sp_BlankInsert', 'sp_BlankSelect', 'sp_BlankUpdate',
    'sp_CreateBackupTable', 'sp_displayProcedures', 'sp_DisplayProcLog', 'sp_DisplaySQLHistory',
    'sp_FinalDrop', 'sp_FirstDrop', 'sp_GetBackupDate', 'sp_GetCreateTableSQL', 'sp_GetDropDate',
    'sp_GetProcedureDefinition', 'sp_GetProcedureParameters', 'sp_GetTableRowCount', 'sp_LoadSystemObjects',
    'sp_LogActivity', 'sp_LogSQL', 'sp_LogStoredProcActivity', 'sp_RemoveSQL', 'sp_RestoreDrop',
    'sp_RestoreTable', 'sp_ShowTableRows','sp_ShowTables');

function DebugP($desc, $text){
    if(WPDB_DEBUG == true){
        echo 'DESC: ' . $desc . '<br />Out: ' . $text . ' #END<br />';
    }
}

function GetParamType($theParam){
    $theDataType = '';
    $dataTypes = array('HAR', 'EXT', 'SON', 'ATETIME', 'ATE', 'IME', 'EAR', 'NT', 'OOLEAN',
        'EC', 'UMBER', 'LOAT', 'OUBLE');
...
function GetFriendlyWPDBName($thisTable){
    $returnName = '';
    if(strpos(strtolower($thisTable), 'wpdb_backup_log') > 0) $returnName = 'WPDB Backup Log';
    if(strpos(strtolower($thisTable), 'wpdb_activity_log') > 0) $returnName = 'WPDB Activity Log';
    if(strpos(strtolower($thisTable), 'wpdb_objects') > 0) $returnName = 'WPDB Objects';
    if(strpos(strtolower($thisTable), 'wpdb_sql_log') > 0) $returnName = 'WPDB SQL Log';
    if(strpos(strtolower($thisTable), 'wpdb_restore_log') > 0) $returnName = 'WPDB Restore Log';
    if(strpos(strtolower($thisTable), 'wpdb_drop_table_log') > 0) $returnName = 'WPDB Drop Table Log';
    if(strpos(strtolower($thisTable), 'wpdb_sp_activity_log') > 0) $returnName = 'WPDB Stored Proc Activity Log';
    return $returnName;
}
...
function isCurrentProc($theSQL){
    ...
    $procedureExists = 0;
    $sql = 'CALL sp_displayProcedures("' . DB_NAME . '");';
...
function isProtectedProc($theSQL){
    ...
    for($i = 0; $i < count($protectedProcs); $i++){
        if(strpos($theSQL, $protectedProcs[$i]) > 0){
            $isProtected = 1;
            break;
        }
    }
